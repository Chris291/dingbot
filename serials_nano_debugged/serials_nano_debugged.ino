...
#define RECEIVE_STATIC_REQUEST 't'
...
String strReceived;
char angularCommand[LENGTH_HEX_NUM_DIGITS];
char command;
...
void readSerial() //receive characterizing prefix (+ length in 2 digit Hex, with manipulation of first bit for sign)
{
  if (Serial.available() > 0) {
    command = Serial.read();
    if (command == RECEIVE_ANGLE_CMD) { //a
      if (firstTimeCommand) {
        enableServo = 1;
        firstTimeCommand = 0;
      }
...
void readAngularChange() {
  if (angularCommand[0] > ASCII_MIDDLE_POINT) {
    angularCommand[0] -= ASCII_DIFFERENCE;
    positive = 0;
  }
  else positive = 1;

  if (positive) {
    angularChangeReceived = strtol(angularCommand, 0, 16);
  }
  else angularChangeReceived = -strtol(angularCommand, 0, 16);
}
...
void sendFeedback() {
  ...
  */
  ...
  Serial.print(sendingFeedback[1]);
  Serial.flush();
}
